...
tripRouter.post("/add", async (req, res) => {
    ...
    const totaltimes = Number(totaltime.slice(0, 2)) * 60 + Number(totaltime.slice(3))
    try {
        const newtrip = new TripModel({ name, from, to, busid, journeystartdate: startDate, journeyenddate: endDate, starttime: startimetotal, endtime: endimetotal, totaltime:totaltimes, price, distance, totalseats, bookedseats: 0, availableseats: totalseats })
        await newtrip.save()
