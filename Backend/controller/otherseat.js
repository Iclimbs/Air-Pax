...
OtherSeatRouter.post("/selectedseats", async (req, res) => {
    const { primaryuser, passengerdetails, tripId, bookingRefId } = req.body
    ...
    for (let index = 0; index < passengerdetails.length; index++) {
        ...
        seatdetails.push({
            ...
            pnr: ticketpnr,
            details: { fname: passengerdetails[index].fname, lname: passengerdetails[index].lname, age: passengerdetails[index].age, gender: passengerdetails[index].gender, phoneno:passengerdetails[index].phoneno,email:passengerdetails[index].phoneno,country:passengerdetails[index].country,seatno:passengerdetails[index].seatno }
        })
    }
